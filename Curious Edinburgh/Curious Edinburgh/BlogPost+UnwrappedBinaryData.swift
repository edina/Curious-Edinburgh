...
extension BlogPost: MKAnnotation {
        ...
        get {
            ...
            if let rawTitle = self.titleJsonData{
                if let titleUnwrapped = NSKeyedUnarchiver.unarchiveObjectWithData(rawTitle){
                    ...
                    title = titleJson["rendered"].stringValue.stringByDecodingHTMLEntities
                }
            }
            return title;
        }
    ...
    var tourNumber: String? {
        get {
            ...
            if let customFields = NSKeyedUnarchiver.unarchiveObjectWithData(self.customFields!){
                let customFieldsJson = JSON(customFields)
                if let tour = defaults.stringForKey(Constants.UseDefaults.tour) {
                    tourNumber = customFieldsJson[tour].string
                } else {
                    tourNumber = customFieldsJson[Constants.defaultTour].string
                }
            }
            return tourNumber
        }
...
extension String {
    ...
    // Taken from https://gist.github.com/jrmgx/3f9f1d330b295cf6b1c6
